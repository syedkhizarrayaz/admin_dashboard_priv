  ...
  return (
    <div className="ml-3 h-screen md:overflow-hidden overflow-auto pb-10">
      {activeMenu && (
        <>
          <div className="flex justify-between items-center">
            <Link to="/Users" className="flex items-center gap-3 ml-3 mt-4 text-xl font-extrabold tracking-tight text-white" onClick={handleCloseSidebar}>
              <SiShopware size={24} /> <span>Admin Panel</span>
            </Link>
            <button
              type="button"
              onClick={() => setActiveMenu(!activeMenu)}
              className="text-xl p-2 md:p-3 hover:bg-light-gray mt-4 block md:hidden"
            >
              ...
              <div key={item.title}>
                ...
                {item.links.map((link) => (
                  <NavLink
                    ...
                    key={link.name}
                    onClick={handleCloseSidebar}
                    className="flex items-center gap-3 pl-4 pt-3 pb-2.5 text-md text-gray-700 dark:text-gray-200 dark:hover:text-black hover:bg-light-gray m-2 rounded-lg"
